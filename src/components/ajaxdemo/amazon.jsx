...
function Amazon() {
    ...
    function LoadData(){
            // xml http and json both are workig fine here
            // var http = new XMLHttpRequest()
            // http.open('get', 'amazon.json', true)
            // http.send()
            // http.onreadystatechange = function(){
            //     if (http.readyState == 4){
            //         amazonSetProduct(JSON.parse(http.responseText));
            //     }
            // }

            fetch('amazon.json')
            .then((response) => response.json())
            .then((amazon) => {amazonSetProduct(amazon)})
    }
